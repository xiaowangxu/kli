import { Align, BoxSizing, FlexDirection, Justify, Overflow, PositionType, Wrap } from "yoga-layout";
import { Container, TextContainer, TextWrap } from "./package/node/container.js";
...
import { defineComponent, ref, reactive, h, onMounted, onUnmounted, nextTick, effect } from 'vue'
import { Box } from "./package/vue/component/Box.js";
import { TextBox } from "./package/vue/component/TextBox.js";
import { Txt } from "./package/vue/component/Txt.js";
import { createKliApp } from "./package/vue/vue_render.js";
import { Br } from "./package/vue/component/Br.js";
import { Span } from "./package/vue/component/Span.js";

const BoxComp = defineComponent({
    name: 'BoxComp',
    setup() {
        const count = ref(0);
            ...
            return Box(
                {
                    height: 5,
                    border: 1,
                    border_color: Color.of(240, 150, 100),
                    border_type: BorderStyleType.Round,
                    padding_horizontal: 1
                },
                [
                    TextBox(undefined,
                        Txt(undefined, count.value.toString())
                    )
                ]
            )
        };
        ...
        return () => Box(
            ...
            [
                Box(
                    {
                        border: 1,
                        border_color: Color.of(190, 255, 0),
                        border_type: BorderStyleType.Round,
                        flex_basis: '50%',
                        flex_grow: 1,
                        flex_shrink: 1,
                    ...
                    [
                        //                         TextBox(undefined, Txt(undefined,
                        //                             [
                        //                                 "Hello World",
                        //                                 Br(),
                        //                                 Span({ color: Color.of(255, 0, 0), bold: true, italic: true }, "from Kli"),
                        //                                 Br(),
                        //                                 `Hello World 这个可以换行 😘
                        // 这是个非常好的问题，实际上是 **终端字符宽度（character width / display width）** 的问题。
                        // 你看到中文引号 \`“ ”\` 在控制台中宽度为 **1**，是因为 **Unicode East Asian Width 属性** 的定义和终端渲染策略不一致造成的。下面我来详细解释：
                ...
                Box(
                    {
                        overflow: Overflow.Hidden,
                        border: 1,
                        border_color: Color.of(12, 190, 230),
                        border_type: BorderStyleType.Round,
                        flex_grow: 0,
                        flex_shrink: 0,
                        width: 25,
                    ...
                    new Array(10).fill(null).map(_ => h(BoxComp)),
                )
            ]
        );
    }
